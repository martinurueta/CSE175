...
def uniform_cost_search(problem, repeat_check=False):
    ...
    if problem.is_goal(Locate.loc): # if node contains a goal state of problem then return node
        return Locate
    Frontier_queue = Frontier(Locate, sort_by ='g') # initialize the frontier to contain node # For the frontier, use a priority queue that sorts by g(n) cost.
        ...
        for node in templist: # for each child of node
            if (repeat_check == True): # checking if true, and find node in list, should not add it to frontier
                if (node in reach_set): # if child is in the reached set
                    if ((node in Frontier_queue )and (Frontier_queue[node] > node.value("g"))): # if child is in frontier but with a higher cost
