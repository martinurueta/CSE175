...
# Martin Urueta - 11/27/2022
#
# Colaborators: Angelo Fatali
#
...
def expected_value_over_delays(state, ply): 
    ...
    # This was take off the lecture 11 slides uncertainty
    # E(A|S) = sum(P(Result(A,S) | S ^ Do(A)) * U(Result(A,S)))  - Decision Theory
    for i in range(2, 6):
...
def heuristic_value(state):
    ...
    # PLACE YOUR CODE HERE
    # used location to determine the value of the state and as well as the board size
    # heurstic function for player is lower val
    # heurstic function for computer is higher val
